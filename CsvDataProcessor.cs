...
public class CsvDataProcessor
{
    ...
    public CsvDataProcessor(long expectedGrossDie = 0) // 修改构造函数，接收 expectedGrossDie
    {
        ...
        _expectedGrossDie = expectedGrossDie; // 初始化 _expectedGrossDie
    }
    ...
    public Dictionary<string, List<FileInfo>> GetSortedCsvFiles(string folderPath)
    {
        ...
        // 过滤出需要合并的组：即 WaferId 和 MergeKey 相同，但 RpValue 不同的组
        var filesToMerge = new Dictionary<string, List<FileInfo>>();
        foreach (var group in groupedFiles)
        {
            var distinctRpValues = group.Value.Select(p => p.RpValue).Distinct().ToList();
            if (distinctRpValues.Count > 1) // 存在多个不同的RP值，需要合并
            {
                // 将 ParsedCsvFileInfo 转换回 FileInfo 并添加到结果字典
                filesToMerge[group.Key] = group.Value.Select(p => p.OriginalFile).ToList();
            }
        }
        ...
        foreach (var file in csvFiles)
        {
            ...
            try
            {
                using (StreamReader reader = new StreamReader(file.FullName))
                {
                    ...
                    {
                        Console.WriteLine($"警告：文件 {file.Name} 在前 {maxHeaderScanLines} 行中未找到 X_COORD 或 Y_COORD 列的标题，跳过。");
                        continue;
                    }
            ...
            {
                Console.WriteLine($"处理文件 {file.Name} 时发生错误: {ex.Message}");
            }
        }
            ...
            using (StreamReader reader = new StreamReader(file.FullName))
            {
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    if (coordinateHeaderFound && secondaryCoordinateHeaderFound && headerIndices.Any())
                    {
                        ...
                        {
                            Console.WriteLine($"警告：文件 '{file.Name}' 中未找到X_COORD或Y_COORD列。跳过该文件。");
                            break; // 跳过该文件
                        }
    ...
    public void CalculateAndAggregateSummaryData()
    {
        ...
        if (_expectedGrossDie > 0 && aggregatedCounts.TryGetValue("Total", out long totalCount))
        {
            ...
            {
                Console.WriteLine($"警告：汇总数据的 Total 值 ({totalCount}) 与 devices.ini 中的预期 GrossDie ({_expectedGrossDie}) 不符！");
                Environment.Exit(1); // 退出程序
            }
            else
            {
                Console.WriteLine($"验证成功：汇总数据的 Total 值 ({totalCount}) 与 devices.ini 中的预期 GrossDie ({_expectedGrossDie}) 相符。");
            }
        }
    }
